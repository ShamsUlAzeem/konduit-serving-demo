...
class BmiDataset(Dataset):
    def __init__(self, path):
        ...
        self.sig_paths = pathlib.Path(path).glob('S_bmi_*.npy')
        self.sig_paths = sorted([x for x in self.sig_paths])
...
def write_batch(batch_num, faces, bmis, sig, out_path):
    ...
    Sig_batch = np.asarray(sig, dtype='float32')
    Sig_batch = Sig_batch.reshape(Sig_batch.size, 1)


    face_file = os.path.join(out_path, 'X_%03d.npy' % (batch_num,))
    bmi_file = os.path.join(out_path, 'y_bmi_%03d.npy' % (batch_num,))
    Sig_file = os.path.join(out_path, 'S_bmi_%03d.npy' % (batch_num,))

    np.save(face_file, face_batch)
    np.save(bmi_file, bmi_batch)
    np.save(Sig_file, Sig_batch)
...
def extract_labels(image_path):
    ...
    if (bmi >= 10) & (bmi < 18.5):
        ...
        bmi_sig = (bmi - 10)/(18.5 - 10)
        bmi = 0

    elif (bmi >= 18.5) & (bmi < 25):

        bmi_sig = (bmi - 18.5)/(25 - 18.5)
        bmi = 1

    elif (bmi >= 25) & (bmi < 30):

        bmi_sig = (bmi - 25) / (30 - 25)
        bmi = 2

    elif (bmi >= 30) & (bmi < 35):

        bmi_sig = (bmi - 30) / (35 - 30)
        bmi = 3

    elif (bmi >= 35) & (bmi < 40):

        bmi_sig = (bmi - 35) / (40 - 35)
        bmi = 4

    elif (bmi >= 40) & (bmi < 45):

        bmi_sig = (bmi - 40) / (45 - 40)
        bmi = 5

    else:
        bmi_sig = (bmi - 45) / (120 - 45)
        bmi = 6
